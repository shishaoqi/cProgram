...
static void ExecuteCommand(bufferADT buffer, string line)
{
	...
	if(toupper(line[0])=='I')
			...
			switch(toupper(line[0]))
		{
			...
			{
				case 'F': MoveCursorFWord(buffer); break;
				case 'B': MoveCursorBWord(buffer); break;
				case 'I': 
					{
						printf("\nInput the word that you want insert: ");
						words=GetLine();
						printf("%s\n",words);
						InsertWord(buffer, words); break;
					}
				case 'D': DeleteWord(buffer); break;
			}
		}
}
