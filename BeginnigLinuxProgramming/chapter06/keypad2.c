...
/*  Next, we must turn echo off
    ...
    while(key != ERR && key != 'q') {
	    ...
	    case KEY_HOME: printw("%s", "HOME key"); break;
	    case KEY_BACKSPACE: printw("%s", "BACKSPACE key"); break;
	    case KEY_DL: printw("%s", "delete-line key"); break;
	    case KEY_IL: printw("%s", "insert-line key"); break;
